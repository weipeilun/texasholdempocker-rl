...
class One2ManyQueue(AbstractQueue):
    ...
    def put(self, data):
        ...
        assert isinstance(data[2], np.ndarray) or (isinstance(data[2], (list, tuple)) and len(data[2]) == len(self.shms_np_list[0][0]) and all([isinstance(item, np.ndarray) for item in data[2]])), 'Only ndarray data is supported.'
