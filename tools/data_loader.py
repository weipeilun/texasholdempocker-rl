...
def train_data_generator(filename, epoch=1):
    ...
    while True:
        ...
        if feature_str == '' or target_probs_str == '' or target_Qs_str == '' or target_mask_str == '' or target_value_str == '':
        ...
        yield modified_feature_list, modified_target_probs_list, modified_target_Qs_list, modified_target_mask_list, modified_target_winning_prob
    f.close()
...
def data_batch_generator(filename, batch_size=8, epoch=1):
    ...
    while True:
        ...
        feature_list, target_probs_list, target_Qs_list, target_mask_list, target_winning_prob = data_generator.__next__()
        ...
        if len(feature_list_buffer) == batch_size:
            yield feature_list_buffer, target_probs_list_buffer, target_Qs_buffer, target_mask_buffer, target_winning_prob_buffer
            feature_list_buffer = list()
            target_probs_list_buffer = list()
            target_Qs_buffer = list()
            target_mask_buffer = list()
