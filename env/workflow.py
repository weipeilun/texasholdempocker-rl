    ...
    def queue_monitor_thread(in_queue, send_in_queue, out_queue, logging_queue, pid):
        ...
        while True:
            ...
            for idx, (over_process_idx, _) in out_queue.thread_id_map.items():
                out_queue_data_num_list[over_process_idx] += not out_queue.signal_slots[idx]
...
def training_thread(model, model_path, step_counter, is_save_model, eval_model_queue, first_train_data_step, train_per_step, update_model_per_train_step, eval_model_per_step, log_step_num, historical_data_filename, game_id_counter, seed_counter, env_info_dict, game_id_signal_queue, num_game_loop_thread, train_update_model_signal_queue, train_hold_signal_queue_list, train_hold_signal_ack_queue):
    ...
    if os.path.exists(historical_data_filename):
        ...
        for observation_list, action_probs_list, action_masks_list, reward_value_list, card_result_value_list, player_winning_prob_bin_list, opponent_winning_prob_bin_list in data_generator:
            model.store_transition(observation_list, action_probs_list, action_masks_list, reward_value_list, card_result_value_list, player_winning_prob_bin_list, opponent_winning_prob_bin_list, save_train_data=False)
